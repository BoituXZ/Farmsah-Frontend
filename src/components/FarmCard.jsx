...
const FarmCard = ({
    ...
    const fetchLocationName = async () => {
      ...
      try {
        ...
        if (data?.address) {
          ...
          const name = [road, city || town || village, country]
            .filter(Boolean)
            .join(", ");
  ...
  // Only show the first content before the second comma in displayLocation
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/user/farms/${slug}`,
        {
          method: "DELETE",
          credentials: "include",
        }
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {getShortLocation(displayLocation)}
          </Typography>
        </Stack>
        <Box
          sx={{
        ...
        >
          ...
          <Typography variant="body1">
            <strong>Livestock:</strong> {livestock}
          </Typography>
        </Stack>
      </Paper>
      ...
      <Modal open={open} onClose={handleClose}>
        ...
        >
          ...
          >
            ...
            <Box>
              <IconButton title="Edit" onClick={() => setIsEditing(!isEditing)}>
                <Edit />
              </IconButton>
              <IconButton
                title="Delete"
          ...
          >
            {!isEditing ? (
              <Stack spacing={2} sx={{ py: 2 }}>
                <Typography>
                  <strong>Farm Name:</strong> {newFarmName}
                </Typography>
                <Typography>
                  <strong>Location:</strong> {getShortLocation(displayLocation)}
                </Typography>
                <Typography>
                  <strong>Size:</strong> {newSize}
                </Typography>
                <Typography>
                  <strong>Crops:</strong> {newCrops}
                </Typography>
                <Typography>
                  <strong>Livestock:</strong> {newLivestock}
                </Typography>
              </Stack>
            ) : (
              <Stack spacing={2}>
